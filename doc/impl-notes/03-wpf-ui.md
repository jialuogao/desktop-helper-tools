...
- `EnsureTopmost` atomically forces the window to the top of the topmost Z-order band via Win32 `SetWindowPos(HWND_TOPMOST)`, replacing the fragile WPF `Topmost` property toggle (`false→true` demote-promote dance; WPF setter does call `SetWindowPos` for shown windows but requires bypassing a short-circuit and briefly demotes). Called at startup (`SourceInitialized`) and by a five-second, no-activation watchdog; each watchdog tick is one `SetWindowPos` call with no focus activation or logging. The `WM_WINDOWPOSCHANGING` hook forces `WINDOWPOS.hwndInsertAfter = HWND_TOPMOST` when Windows explicitly repositions our window. `SystemEvents.PowerModeChanged` re-asserts topmost immediately and again after 500 ms and 2 seconds because the first resume notification can arrive before the desktop has finished rebuilding its window bands.
- `HWND_TOPMOST` places the overlay above the non-topmost band but does not create an absolute z-order monopoly over other topmost windows. The message hook does not guarantee notification when another topmost window is inserted above the overlay; the watchdog and interaction paths provide eventual reassertion, while exclusive-fullscreen and other system-managed surfaces remain outside this guarantee.

...
- `TrySetResolution` first confirms the requested width/height pair appears in the enumerated support list, then asks `SetDisplayConfig` to validate and apply the source mode across the complete active topology. With virtual-mode-aware paths, the source mode index is the high 16 bits of the source-info union; the low 16 bits carry the clone group. When the queried path reports an invalid source index, the implementation associates the source mode by adapter LUID plus source ID and rewrites the path index before submission. If the CCD path is unavailable or rejected, it falls back to the legacy `DEVMODEW`/`ChangeDisplaySettingsExW` call after passing a `CDS_TEST` preflight check and verifying post-apply read-back resolution.
...
- Failures return `false`, preserve a human-readable `LastError`, and log the native return code. `DisplayApi.ClearLastError()` is called before submitting changes to prevent stale error text from leaking into subsequent verification steps. The UI maps failures to an actionable dialog instead of swallowing them.
- On resolution toggle success, `ResolutionSwitcher` records `LastResolutionChange` containing `OldBounds` and `NewBounds`. `OverlayWindow.ApplyResolutionChange` proportionally scales the floating button's relative coordinates when it is on the target display (e.g. 2560→1920 keeps relative X/Y scaling so it does not spill onto adjacent displays) or adjusts bounds offsets when it is on an adjacent display.
